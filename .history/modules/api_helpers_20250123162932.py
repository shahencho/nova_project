...
def generate_financial_status_message(code, deposit, debt, apartment, type_of_object, flat_number):
    ...
    message = (
        f"Գույք համար: {code}\n"
        f"Շենք: {apartment}\n"
        f"{type_of_object}Բնակարան: {flat_number} ({type_of_object})\n\n"
    )

    # Debt and deposit details
    if debt > 0 and deposit > 0:
        ...
        message += (
            f"Դուք ունեք պարտք՝ {debt}:\n"
            "Խնդրում ենք հնարավորինս շուտ կատարել վճարումը։\n"
        )
    elif deposit > 0:
        message += f"Դուք ունեք կանխավճար՝ {deposit}։ Շնորհակալություն ժամանակին վճարումները կատարելու համար։\n"
