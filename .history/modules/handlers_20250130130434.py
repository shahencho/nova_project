...
from api_helpers import fetch_deposit_and_debt_from_api, fetch_customer_properties, fetch_debt_for_code, fetch_property_details, generate_financial_status_message, fetch_auth_token
...
async def show_payment_history(code, update):
    ...
    if details and isinstance(details, dict):
        ...
        if transactions:
            history = "\n".join([
                f"{trans['payedDate']}: {format_number(trans['payedAmount']}_" for trans in transactions
            ])
