...
def generate_financial_status_message(code, deposit, debt, apartment, type_of_object, flat_number):
    ...
    if debt < 0:
        message += (
            f"Դուք ունեք պարտք՝ {abs(debt)}։ Խնդրում ենք հնարավորինս շուտ կատարել վճարումը։\n"
        )
        ...
        message += (
            f"Դուք ունեք կանխավճար՝ {debt}։ Շնորհակալություն ժամանակին վճարումները կատարելու համար։\n"
        )
