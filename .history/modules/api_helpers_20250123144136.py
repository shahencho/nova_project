...
def generate_financial_status_message(code, deposit, debt):
    ...
    if debt > 0:
        message = (
            f"{code} Դուք ունեք պարտք, պարտքի գումարը՝  {debt}. ։ Խնդրում ենք հնարավորինս շուտ կատարել վճարումը։\n"
        ...
        message = (
            f"{code} Դուք ունեք կանխավճար, գումարը՝  = {deposit}. ։ Շնորհակալություն ժամանակին վճարումները կատարելու համար։\n"
        )
    else:
        message = (
            f"{code} Ձեր հաշվեկշիռը նորմալ է։ Մնում է զրոյական հաշվեկշիռ։\n"
        )
